...
public class DavResourceFinder {
    ...
    public DavResourceFinder(Logger log, Context context, ServerInfo serverInfo) {
        ...
        if(serverInfo.getAccessToken() != null) {
            httpClient = new HttpClient(log, context, serverInfo.getAccessToken());
        } else {
    ...
    public void findResources(Service service) throws URISyntaxException {
        ...
        if (service == Service.CALDAV) {
            calendars.clear();
            taskLists.clear();
            if(serverInfo.getCaldavURI() != null)
                baseURI = new URI(serverInfo.getCaldavURI());
        } else if (service == Service.CARDDAV) {

            addressbooks.clear();
            if(serverInfo.getCarddavURI() != null)
                baseURI = new URI(serverInfo.getCarddavURI());
        }
