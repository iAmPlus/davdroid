...
public abstract class AndroidEvent {
    ...
    protected void populateEvent(ContentValues values) {
        ...
        if (allDay) {
            ...
            java.util.TimeZone vTimezone = null;
            Log.i(TAG, "Event timezone " + tzId);
            /*Timezone needs to be added along with time for correct detection of event start/stop time
            Can use below code to convert long name timezone to Oslon ID if not set properly by calendar
            boolean foundTZ=false;
            for (String availId : TimeZone.getAvailableIDs()){
                if(tzId.equalsIgnoreCase(availId) || tzId.equalsIgnoreCase(TimeZone.getTimeZone(availId).getDisplayName())) {
            ...
            if(foundTZ) {
                tzId = vTimezone.getID();
                Log.i(TAG, "Setting timezone to " + tzId);
            }
            Log.i(TAG, "Event timezone " + tzId);*/
